...
out_dir         = f'output_needlet_TG/EUCLID/Mask_noise/TG_{nside}_nsim{nsim}_mask_smooth/'
...
plt.suptitle(r'Euclid Mask smooth NEEDLETS $D = %1.2f $' %myanalysis.B +r'$ ,~j_{\mathrm{max}} =$'+str(jmax) + r'$ ,~\ell_{\mathrm{max}} =$'+str(lmax) + r'$ ,~N_{\mathrm{side}} =$'+str(simparams['nside']) + r',$~f_{sky} = $'+str(fsky))
...
def cov_pseudo_cl(cltg,cltt, clgg, wl, lmax, noise_gal_l=None):
    ...
    return covll
cls_tg = np.loadtxt('/ehome/bdecaro/xcmbneed/src/cls_from_maps/EUCLID/Euclid mask smooth/cls_Tgalnoise_anafast_nside128_lmax256_Euclidnoise_Marina_nsim1000_fsky0.37.dat')
...
plt.suptitle(r'Euclid Mask smooth PCL $ ,~\ell_{\mathrm{max}} =$'+str(lmax) + r'$ ,~N_{\mathrm{side}} =$'+str(simparams['nside']) + r',$~f_{sky} = $'+str(fsky))
...
ax.set_ylabel(r'% diag(sim cov)/diag(analyt cov)-1')
fig.tight_layout()
plt.savefig(out_dir+f'diff_cov_pcl_theory_T_gal_lmax{lmax}_nsim{nsim}_nside{nside}_mask.png', bbox_inches='tight')
#######################################################################################################################
...
fig.tight_layout()
plt.savefig(out_dir+f'diff_gammaj_mean_theory_over_sigma_jmax_{jmax}_B{myanalysis.B:1.2f}_nsim{nsim}_nside{nside}.png')
...
fig.tight_layout()
plt.savefig(out_dir+f'SNR_cumulative_betaj_theory_T_gal_jmax{jmax}_D{myanalysis.B:0.2f}_nsim{nsim}_nside{nside}_mask.png', bbox_inches='tight')
