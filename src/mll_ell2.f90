...
contains 
  ...
  subroutine get_Mll(wl, lmax, mll)
    ...
    real(dp), intent(in), dimension(2:lmax)  :: wl
    real(dp), intent(out), dimension(2:lmax, 2:lmax) :: mll

    real(dp), parameter  :: one_fourpi = 1.d0 / (atan(1.) * 16.d0)
    integer :: l1, l2, l3min, l3max, dim, ier, l, lwig
    real(dp) :: rl1, rl2, rl3min, rl3max, numb
    real(dp), dimension(2:2*lmax) :: wigner
    real(dp), dimension(2:lmax)   :: ell
    write(*,*)"sono dentro mll_ell2"
    wigner = 0.
