...
nbins = 3
# Paths
fname_xcspectra = '/ehome/bdecaro/xcmbneed/src/spectra/inifiles/fiducial_EUCLID_tomo_nbins3_only_w1w1_w2w2_w3w3.dat'#'spectra/inifiles/CAMBSpectra_planck.dat' 
...
Nll = np.ones(xcspectra.clgg[0].shape[0])/simparams['ngal']
...
for bin in range(nbins):
    print(f'Bin={bin}')
    betaj_sims_TS_galS[bin] = myanalysis.GetBetajSimsFromMaps(f'TS{bin}', nsim, field2=f'galS{bin}', fname=fname_betaj_sims_TS_galS[bin], fsky_approx=True)
    betaj_sims_TS_galS_mask[bin] = myanalysis.GetBetajSimsFromMaps(f'TS{bin}', nsim, field2=f'galS{bin}', mask= mask, fname=fname_betaj_sims_TS_galS_mask[bin], fsky_approx=True)

    # Covariances
    print("...computing Cov Matrices...")
    cov_TS_galS[bin], corr_TS_galS[bin]         = myanalysis.GetCovMatrixFromMaps(field1=f'TS{bin}', nsim=nsim, field2=f'galS{bin}', fname=fname_cov_TS_galS[bin], fname_sims=fname_betaj_sims_TS_galS[bin])
    cov_TS_galS_mask[bin], corr_TS_galS_mask[bin]          = myanalysis.GetCovMatrixFromMaps(field1=f'TS{bin}', nsim=nsim, field2=f'galS{bin}', fname=fname_cov_TS_galS_mask[bin], mask = mask,fname_sims=fname_betaj_sims_TS_galS_mask[bin])

    print("...done...")

# <Beta_j>_MC
    betaj_TS_galS_mean[bin]    = myanalysis.GetBetajMeanFromMaps(f'TS{bin}', nsim, field2=f'galS{bin}', fname_sims=fname_betaj_sims_TS_galS[bin])
    betaj_TS_galS_mask_mean[bin]     = myanalysis.GetBetajMeanFromMaps(f'TS{bin}', nsim, field2=f'galS{bin}', fname_sims=fname_betaj_sims_TS_galS_mask[bin])
...
for bin in range(nbins):
    ...
    delta[bin] = need_theory.delta_beta_j(jmax, cltt = xcspectra.cltt, cltg = xcspectra.cltg[bin], clgg = xcspectra.clgg[bin])
...
for bin in range(nbins):
    ...
    ax.errorbar(myanalysis.jvec[1:jmax], betaj_TS_galS_mean[bin][1:jmax], yerr=np.sqrt(np.diag(cov_TS_galS_mask[bin])[1:jmax])/np.sqrt(nsim) ,fmt='o',ms=5,capthick=2, label=f'Error of the mean , Bin = {bin}')
    #ax.errorbar(myanalysis.jvec[1:jmax], betaj_TS_galS_mean[bin][1:jmax], yerr=np.sqrt(np.diag(cov_TS_galS_mask[bin])[1:jmax]) ,color='grey',fmt='o',ms=0,capthick=2, label=f'Error of simulations, Bin = {bin}')
...
for bin in range(nbins):
    ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galS_mask_mean[bin][1:jmax] -betatg[bin][1:jmax])/betatg[bin][1:jmax], yerr=np.sqrt(np.diag(cov_TS_galS_mask[bin])[1:jmax])/(np.sqrt(nsim)*betatg[bin][1:jmax]),  fmt='o',  ms=5,label=f'Variance on the mean , Bin = {bin}')
    ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galS_mask_mean[bin][1:jmax] -betatg[bin][1:jmax])/betatg[bin][1:jmax], yerr=delta[bin][1:jmax]/(np.sqrt(nsim)*betatg[bin][1:jmax]),  fmt='o',  ms=5,label=f'Variance from theory, Bin = {bin}')
...
#s2n_mean_sim=S_2_N_th(betaj_TS_galS_mask_mean, delta**2)
#s2n = S_2_N_sum(betaj_TS_galS_mean, delta**2)
#
#print(f's2n={s2n:0.2f}')
#
#fig = plt.figure(figsize=(17,10))
#
#plt.suptitle(r'$D = %1.2f $' %B +r'$ ,~j_{\mathrm{max}} =$'+str(jmax) + r'$ ,~\ell_{\mathrm{max}} =$'+str(lmax) +r', $f_{\mathrm{sky}}$ = %1.2f'%fsky+ r'$ ,~N_{\mathrm{side}} =$'+str(simparams['nside']) + r',$~N_{\mathrm{sim}} = $'+str(nsim))
...
#
#fig.tight_layout()
#plt.savefig(out_dir+f'SNR_betaj_theory_T_gal_jmax{jmax}_D{B:0.2f}_nsim{nsim}_nside{nside}_mask.png', bbox_inches='tight')
#
#
#