...
ax0.errorbar(myanalysis.jvec[1:jmax], betaj_TS_galT_mask_mean[1:jmax]/fsky, yerr=delta_noise[1:jmax]/(np.sqrt(nsim)), color='firebrick', fmt='o', ms=10,capthick=5, label=r'Variance from theory')
...
ax.axhline(ls='--', color='k')
ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galT_mask_mean[1:jmax]/fsky )/(betatg[1:jmax])-1, yerr=delta_noise[1:jmax]/(np.sqrt(nsim)*betatg[1:jmax]), color='firebrick', fmt='o',  ms=10,capthick=5, label=r'Variance from theory')
