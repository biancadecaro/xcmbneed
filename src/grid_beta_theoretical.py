...
cov_dir         = []
for om in OmL:
    if not os.path.exists(f'output_needlet_TG_OmL/Grid_spectra_{len(OmL)}_old/TGsims_theoretical_OmL{om}/'):
        os.makedirs(f'output_needlet_TG_OmL/Grid_spectra_{len(OmL)}_old/TGsims_theoretical_OmL{om}/')
    fname_xcspectra.append(f'spectra/Grid_spectra_{len(OmL)}_no_mnu_old/CAMBSpectra_OmL{om}_lmin0.dat')#.replace('.', '') np.chararray(len(OmL))Grid_spectra_{len(OmL)}s
    out_dir.append(f'output_needlet_TG_OmL/Grid_spectra_{len(OmL)}_old/TGsims_theoretical_OmL{om}/' )#np.chararray(len(OmL))
