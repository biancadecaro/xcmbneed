...
simparams = {'nside'   : 256,
             ...
             'ngal_euclid'    : 354543085.80126834,
 	     	 'ngal_dim': 'ster',
	     	 'pixwin'  : False}
...
out_dir         = f'output_needlet_TG/Planck/Mask_noise/TG_{nside}_lmax{lmax}_tesi/'
...
#Nll_euclid = np.ones(xcspectra.clg1g1.shape[0])/simparams['ngal_euclid']
#
#ell = np.arange(lmax+1)
#fig = plt.figure(figsize=(17,10))
#ax = fig.add_subplot(1, 1, 1)
#ax.plot((ell*(ell+1)/(2*np.pi))*xcspectra.clg1g1[ell], label = r'$D_{\ell}^{\mathrm{GG}}$')
##ax.plot((ell*(ell+1)/(2*np.pi))*(xcspectra.clg1g1[ell] + Nll[ell]), label = r'$D_{\ell}^{\mathrm{GG}}+N_{\ell}$')
#ax.plot((ell*(ell+1)/(2*np.pi))*(xcspectra.clg1g1[ell] + Nll_euclid[ell]), label = r'$D_{\ell}^{\mathrm{GG}}+N_{Euclid,\ell}$')
#
#ax.legend(loc='best')
#plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
#ax.yaxis.set_major_formatter(formatter) 
#ax.set_xlabel(r'$\ell$')
#ax.set_ylabel(r'$D_{\ell}^{\mathrm{GG}}$')
#plt.savefig(out_dir+f'dl_gg_shot_noise.png')
# Simulations class
...
ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galT_mask_mean[1:jmax] -betatg[1:jmax])/betatg[1:jmax], yerr=np.sqrt(np.diag(cov_TS_galT_mask)[1:jmax])/(np.sqrt(nsim)*betatg[1:jmax]),  fmt='o',  ms=5,label=r'SHOT Variance on the mean from simulations')
#ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galT_mask_mean[1:jmax] -betatg[1:jmax])/betatg[1:jmax], yerr=delta_noise[1:jmax]/(np.sqrt(nsim)*betatg[1:jmax]),color='#6d7e3f',  fmt='o',  ms=5,label=r'Variance from theory')
...
print(100*(betaj_TS_galS_mask_mean[1:jmax] -betatg[1:jmax])/betatg[1:jmax])
...
ax.axhline(ls='--', color='grey')
#ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galT_mean[1:jmax] -betatg[1:jmax])/betatg[1:jmax], yerr=np.sqrt(np.diag(cov_TS_galT)[1:jmax])/(np.sqrt(nsim)*betatg[1:jmax]), fmt='o',ms=5, label=r'Full sky')#, label=r'$T^S \times gal^S$, sim cov')
ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galT_mask_mean[1:jmax]*fsky -gammaJ_tg[1:jmax])/betatg[1:jmax], yerr=fsky*np.sqrt(np.diag(cov_TS_galT_mask)[1:jmax])/(np.sqrt(nsim)*gammaJ_tg[1:jmax]),  fmt='o',  ms=5,label=r'SHOT Variance on the mean from simulations')
...
ax.errorbar(myanalysis.jvec[1:jmax], (betaj_TS_galS_mask_mean[1:jmax]*fsky -gammaJ_tg[1:jmax])/betatg[1:jmax], yerr=fsky*np.sqrt(np.diag(cov_TS_galS_mask)[1:jmax])/(np.sqrt(nsim)*gammaJ_tg[1:jmax]),  fmt='o',  ms=5,label=r'Variance on the mean from simulations')
...
#print(np.sqrt(np.diag(cov_TS_galS))/(np.sqrt(nsim)*betatg) - delta/(np.sqrt(nsim)*betatg))
print('gamma=',100*(betaj_TS_galT_mask_mean[1:jmax]*fsky -gammaJ_tg[1:jmax])/gammaJ_tg[1:jmax])
