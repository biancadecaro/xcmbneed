...
class NeedletTheory(object):
    ...
    def phi_need(self, t):
        ...
        phi = np.zeros(len(t))
        phi[left_idx] = 1.
        phi[cent_idx] = self.psi_need(1.-2.*self.D/(self.B-1.)*(t[cent_idx]-1./self.D))
        phi[rite_idx] = 0.
...
if __name__ == "__main__":
    ...
    need_theory_bianca = NeedletTheory(lmax=lmax,jmax=jmax)
    print(f'D={need_theory_bianca.D}')
    ...
    print(cl_theory_tt.shape)
    betatg_bianca = need_theory_bianca.cl2betaj(jmax, cl=cl_theory_tg)
    delta_bianca = need_theory_bianca.delta_beta_j(jmax=jmax, cltg=cl_theory_tg, cltt=cl_theory_tt,clgg=cl_theory_gg)

    ell_bin = need_theory_bianca.binning(jmax=jmax, lmax=lmax, ell =ell_theory)
    print(ell_bin)
    
    betatg = np.loadtxt('/ehome/bdecaro/xcmbneed/src/output_needlet_TG/EUCLID/Noise/TG_128_nsim100/theory_beta_jmax12_D1.59_lmax256.dat')
    delta  = np.loadtxt('/ehome/bdecaro/xcmbneed/src/output_needlet_TG/EUCLID/Noise/TG_128_nsim100/theory_variance_jmax12_D1.59_lmax256.dat')

    print(f'diff beta={betatg-betatg_bianca}, diff variance={delta-delta_bianca}')

    fig = plt.figure(figsize=(27,20))

    plt.suptitle(r'$D = %1.2f $' %need_theory_bianca.D + r' $\ell_{max} =$'+str(lmax) + r' $j_{max} = $'+str(jmax))
    ax = fig.add_subplot(1, 1, 1)
    ax.errorbar(need_theory_bianca.jvec, betatg,  yerr=delta, color='firebrick', fmt='o', ms=10,capthick=5, label=r'theory')
    ax.legend(loc='best')
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    ax.set_xlabel(r'$j$')
    ax.set_ylabel(r'$\beta_j^{Tgal}$')
    fig.tight_layout()
    plt.savefig(f'betaj_theory_T_gal_noise_jmax{jmax}_D{need_theory_bianca.D:0.2f}_lmax{lmax}.png', bbox_inches='tight')
