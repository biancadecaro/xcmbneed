...
filename_OmL = 'grid_OmL_len_'+str(len(OmL))+'_lmax_'+str(lmax)+'_mnu_1'
...
plt.loglog(ell, data_OmL['nbins1']['cls_grid']['OmL='+str(OmL[4])+'']['W1xW1'][ell], label = 'OmL'+str(OmL[4]))
plt.loglog(ell, data_OmL['nbins1']['cls_grid']['OmL='+str(OmL[8])+'']['W1xW1'][ell], label = 'OmL'+str(OmL[8]))
plt.loglog(ell, data_OmL['nbins1']['cls_grid']['OmL='+str(OmL[16])+'']['W1xW1'][ell], label = 'OmL'+str(OmL[16]))
plt.xlabel(r'$\ell$')
plt.ylabel(r'$\ell(\ell+1)/2\pi C_l^{TG}$')
plt.title(r'nbins='+settings[0]+r', $l_{max}$ ='+str(lmax)+r', OmL ')
plt.legend(loc = 'best' )

plt.savefig('prova_spectra_OmL_grid_30_mnu_W1xW1_1.png')
...
for p, oml in enumerate(OmL):
    filename = '../spectra/Grid_spectra_30_mnu/CAMBSpectra_OmL'+str(oml)+'_mnu_1.dat'#.replace('.', '')+'.dat'
...
filename_fid = '../spectra/CAMBSpectra_fiducial_mnu_1.dat'
