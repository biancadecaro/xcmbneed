...
mask_pl = hp.read_map(f'mask/mask_temp_ns{nside}.fits')#hp.read_map(f'mask/mask_planck_comm_2018_nside={nside}.fits')
...
wl_pl_eu = hp.anafast(map1=mask_pl, map2=mask_eu, lmax=3*nside)
Mll_pl_eu  = need_theory.get_Mll(wl_pl_eu, lmax=3*nside)
wl_comb = hp.anafast(map1=mask_comb, map2=mask_comb, lmax=3*nside)
...
gammaJ_tg_aggiustato = need_theory.gammaJ(cl_theory_tg, Mll_cross, lmax)
delta_gammaj_aggiustato = need_theory.variance_gammaj(cltg=cl_theory_tg,cltt=cl_theory_tt, clgg=cl_theory_gg, Mll_1x2=Mll_comb, Mll=Mll_pl_eu, jmax=jmax, lmax=lmax, noise_gal_l=Nll)
...
cov_TS_galT_mask, corr_TS_galT_mask     = myanalysis.GetCovMatrixFromMaps(field1='T', nsim=nsim, field2='g1noise', mask1=mask_pl, mask2=mask_eu, fname=fname_cov_TS_galT_mask, fname_sims=fname_gammaj_sims_TS_galT_mask)

print("...done...")

# <Gamma_j>_MC
gammaj_TS_galT_mask_mean    = myanalysis.GetBetajMeanFromMaps('T', nsim, field2='g1noise', mask1=mask_pl, mask2=mask_eu, fname_sims=fname_gammaj_sims_TS_galT_mask)
...
ax.errorbar(jvec[1:], (gammaj_TS_galT_mask_mean[1:] /gammaJ_tg[1:]-1)*100, yerr=100*np.sqrt(np.diag(delta_gammaj_aggiustato)[1:])/(np.sqrt(nsim)*gammaJ_tg[1:]),  fmt='o', ms=0, label='Variance of the mean from theory')
...
###########  COMPARISON COUPLING MATRIX ##############################################
fig = plt.figure()

plt.suptitle('Comparison vecchia deltagammaj e nuova')

ax = fig.add_subplot(1, 1, 1)

ax.plot(jvec[1:], (np.diag(delta_gammaj_aggiustato)[1:]/np.diag(delta_gammaj)[1:]-1)*100 ,'o')
...
ax.set_xticklabels(jvec[1:])
ax.set_xlabel(r'$j$')
ax.set_ylabel(r'% $(\Delta \Gamma)^2_{\mathrm{nuova}}/(\Delta \Gamma)^2_{\mathrm{vecchia}}$ - 1')
fig.tight_layout()
...
ax.plot(jvec[1:], (np.diag(cov_TS_galT_mask)[1:]/np.diag(delta_gammaj_aggiustato)[1:]-1)*100 ,'o')
...
ax.plot(myanalysis.jvec[1:], (np.sqrt(np.diag(cov_TS_galT_mask)[1:])/np.sqrt(np.diag(delta_gammaj_aggiustato)[1:])-1)*100 ,'o',color='#2b7bbc')#, label='MASK')
ax.axhline(ls='--', color='grey')
...
cov_pcl_aggiustata= cov_pseudo_cl(cltg=cl_theory_tg,cltt=cl_theory_tt, clgg=cl_theory_gg, Mll=Mll_pl_eu,  Mll_1x2=Mll_comb, lmax=lmax,noise_gal_l=Nll)
...
ax.plot(ell[2:], (np.diag(cov_pcl_sim)[2:]/np.diag(cov_pcl)[2:]-1)*100 ,'o', label='Mll pl eu - Mll Marina')
ax.plot(ell[2:], (np.diag(cov_pcl_sim)[2:]/np.diag(cov_pcl_aggiustata)[2:]-1)*100 ,'o', label= 'Mll pl eu - Mll comb')
ax.axhline(ls='--', color='grey')
ax.set_ylim([-30,30])
plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
ax.yaxis.set_major_formatter(formatter) 
