...
for n,u in enumerate(num):
    print(n,u)
    if field_g_old in str(listdir_out[n]):
    ...
    elif field_T in str(listdir_out[n]):
        name_old_T = f'map_nbin1_NSIDE{nside}_lmax{lmax}_{field_T}_{(u+1):05d}.fits'
        name_new_T = f'sim_{u:04d}_TT_{nside.zfill(4)}.fits'
