...
beta_oml = np.array([np.loadtxt(f'/ehome/bdecaro/xcmbneed/src/output_needlet_TG_OmL/Grid_spectra_30_planck_2_lmin0/TGsims_theoretical_OmL{oml}/beta_TS_galS_theoretical_OmL{oml}_B1.7422102429630426.dat') for oml in OmL])[:,1:(jmax)]#np.array([np.loadtxt(f'/ehome/bdecaro/xcmbneed/src/output_needlet_TG_OmL/Grid_spectra_30_planck_2_lmin0/TGsims_theoretical_OmL{oml}/beta_TS_galS_theoretical_OmL{oml}_B1.7422102429630426.dat') for oml in OmL])[:,1:(jmax)]#np.array([np.loadtxt(f'/ehome/bdecaro/xcmbneed/src/output_needlet_TG_OmL/Grid_spectra_30_planck_2_lmin0/TGsims_512_OmL{oml}/betaj_sims_TS_galS_Om{oml}_jmax12_B = 1.74 _nside512.dat') for oml in OmL])[:,1:(jmax)]
beta_oml_old = np.array([np.loadtxt(f'/ehome/bdecaro/xcmbneed/src/output_needlet_TG_OmL/wrong_grid/Grid_spectra_30_old/TGsims_theoretical_OmL{oml}/beta_TS_galS_theoretical_OmL{oml}_B1.7422102429630426.dat') for oml in OmL])[:,1:(jmax)]
...
delta_old =np.array([np.subtract(beta_fid, beta_oml_old[p]) for p in range(len(OmL))])#beta_fid_old-beta_oml_old#
#print(f'delta={delta[0]}\n', f'delta old={delta_old[0]}')
...
chi_squared_old = liklh.Calculate_chi2_grid(beta_fid=beta_fid, beta_grid=beta_oml_old, cov=cov_matrix, jmax=jmax, params = OmL ) #liklh.delta_chi2_grid(beta_fid=beta_fid_old, beta_grid=beta_oml_old,cov=cov_matrix_variance**2, jmax=jmax, params=OmL)   #
...
print(chi_squared_old/chi_squared)
...
filename = 'Posterior_OmL_'+str(len(OmL))+'_mean_old_newbest-fit_theoretical_OmL_1'#_no_mnu'
