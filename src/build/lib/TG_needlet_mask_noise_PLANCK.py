...
out_dir         = f'/ehome/bdecaro/xcmbneed/src/output_needlet_TG/Planck/Mask_noise/TG_{nside}_mask_shot_noise/'
path_inpainting = 'inpainting/inpainting.py'
cov_dir 		= f'/ehome/bdecaro/xcmbneed/src/covariance/Planck/Mask_noise/covariance_TG{nside}_mask_shot_noise/'
...
ax.errorbar(myanalysis.jvec-0.15, (betaj_TS_galT_mean -betatg)/betatg, yerr=np.sqrt(np.diag(cov_TS_galT))/(np.sqrt(nsim)*betatg), color='firebrick', fmt='o', capsize=0, label=r'$T^S \times gal^T$')
