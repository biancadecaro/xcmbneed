...
def Plot_posterior(posterior, param, filename):#, params):
    ...
    ax.set_title(r'Posterior distribution for $\Omega_{\Lambda}$ , grid = '+str(len(param))+' points')
    ax.set_xlabel(r'$\Omega_{\Lambda}$')
    textstr = '\n'.join((
        r'$\Omega_{\Lambda}=%.2f$' % (mean, ),
        r'$-=%.2f$' % (percentile[0], ),
        r'$+=%.2f$' % (percentile[2], )))
