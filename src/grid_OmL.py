...
if not os.path.exists(f'spectra/Grid_spectra_{len(OmL)}_planck'):
        os.makedirs(f'spectra/Grid_spectra_{len(OmL)}_planck')
filename = f'spectra/Grid_spectra_{len(OmL)}_planck/CAMBSpectra_OmL_fiducial_lmin0.dat'#.replace('.', '')+'.dat'
...
for p, oml in enumerate(OmL):
    filename = f'spectra/Grid_spectra_{len(OmL)}_planck/CAMBSpectra_OmL{oml}_lmin0.dat'#.replace('.', '')+'.dat'
