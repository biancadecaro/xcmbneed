...
C
      ...
      REAL,     intent(in) :: L2, L3, M2, M3
      REAL,     intent(out) :: L1MIN, L1MAX
      REAL,     intent(out), dimension(NDIM) :: THRCOF
C
...
C  Check error conditions 1 and 2.
     ...
     +   (L3-ABS(M3)+EPS.LT.ZERO))THEN
         IER=1
C         CALL XERMSG('SLATEC','RC3JJ','L2-ABS(M2) or L3-ABS(M3) '//
...
C  Check error condition 3.
      IF(MOD(L1MAX-L1MIN+EPS,ONE).GE.EPS+EPS)THEN
         IER=3
c        CALL XERMSG('SLATEC','RC3JJ','L1MAX-L1MIN not integer.',IER,1)
         RETURN
      ENDIF
...
C  Check error condition 4.
      IER=4
c      CALL XERMSG('SLATEC','RC3JJ','L1MIN greater than L1MAX.',IER,1)
      RETURN
C
...
C  Check error condition 5.
   21 IER = 5
c      CALL XERMSG('SLATEC','RC3JJ','Dimension of result array for 3j '//
